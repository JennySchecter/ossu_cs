...
class Point < GeometryValue
  ...
  # double dispatch
  def intersect other
    other.intersectPoint self # Point intersect other, call other's intersectPoint method
  end
  def intersectPoint p
...
class Line < GeometryValue
  ...
  # double dispatch
  def intersect other
    other.intersectLine self # Line intersect other, call other's instersectLine method
  end
  def intersectPoint p
...
class VerticalLine < GeometryValue
  ...
  # double dispatch
  def intersect other
    other.intersectVerticalLine self # VerticalLiine intersect other, call other's intersectVerticalLine method
  end
  def intersectPoint p
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(x1,y1,x2,y2)
      ...
      x = sort_2_number(x1,x2)
      y = sort_2_number(y1,y2)
      LineSegment.new(x[0],y[0],x[1],y[1])
    end
  end
  def shift (dx,dy)
  ...
  # double dispatch
  def intersect other
    other.intersectLineSegment self # LineSegment intersect other 
  end
  def intersectPoint p
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(sx1,sx2) # the segments are on the Vertical Line
      ...
      if real_close(aYend,bYstart)
        ...
        LineSegment.new(x1,bYstart,x1,bYend)  # b inside a
      else
        LineSegment.new(x1,bYstart,x1,aYend) # overlapping
      end
    else # the segments are on the non-vertical line
      if x1 < sx1
        aXstart,aXend,aYstart,aYend = x1,x2,y1,y2
        bXstart,bXend,bYstart,bYend = sx1,sx2,sy1,sy2
      else
        aXstart,aXend,aYstart,aYend = sx1,sx2,sy1,sy2
