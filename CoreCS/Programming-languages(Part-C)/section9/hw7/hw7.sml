...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx,dy,e1) => let val se = eval_prog (e1,env)
			   in
			       case se of
				   NoPoints => se
				 ...
				 | _ => raise Impossible "se must a value" 
			   end			       
(* CHANGE: Add a case for Shift expressions *)

(* helper function compare two number and sort *)
fun sort_2_num (a: real,b: real) =
    if a > b
    then (b,a)
    else (a,b)
...
fun preprocess_prog e =
	...
	NoPoints => e
      ...
      | LineSegment(x1,y1,x2,y2) => if real_close_point (x1,y1) (x2,y2)
					...
					let val x=sort_2_num(x1,x2)			   
					    val y=sort_2_num(y1,y2)
					in
					    LineSegment (#1 x,#1 y,#2 x,#2 y)
					end
