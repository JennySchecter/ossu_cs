...
class Board
  ...
  def initialize (game)
    @grid = Array.new(num_rows) {Array.new(num_columns)}  # 创建一个m*n的数组，初始值均为nil
  ...
  # from the top
  # 当网格的第二行只要有一个不为nil，说明游戏结束 game over
  def game_over?
    @grid[1].any?
  end
  ...
  # rotate方法顺逆时针其实写反了，所以这里的操作实际上产生的实际情况是逆时针。
  # 注意，仔细研究了下，并未写反，方块坐标不是正常的坐标系，而是向下y递增和向右x递增
  ...
  # 当前方块的位置(写入网格之前，也就是移动过程中)，移动一次就消除之前的，重新绘制新的位置
  def draw
