...
class RectangularRoom(object):
    ...
    def getRandomPosition(self):
        ...
        """
        #random.seed(0)
        x = float('%.2f' % (random.random()*self.width))
        y = float('%.2f' % (random.random()*self.height))
        return Position(x,y)
        #raise NotImplementedError
...
class Robot(object):
    ...
    def __init__(self, room, speed):
        ...
        self.path.append((self.position,self.direction))
        #初始降落的地板会被标记为清洁
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        new_position = Position.getNewPosition(self.position,self.direction,self.speed)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        real_coverage = room.getNumCleanedTiles()/room.getNumTiles()
        while (room.getNumCleanedTiles()/room.getNumTiles()) < min_coverage:
            anim.update(room, robots)
            for robot in robots:
                robot.updatePositionAndClean()
            clock_ticks += 1
        clock_tick_list.append(clock_ticks)
        anim.done()
    return sum(clock_tick_list)/len(clock_tick_list)
